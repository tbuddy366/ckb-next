...
RebindWidget::RebindWidget(QWidget *parent) :
    ...
    bind(nullptr), profile(nullptr), macroReader(nullptr)
{
    ui->setupUi(this);
...
void RebindWidget::macroLineRead(QString line, qint64 ustime, bool keydown, bool printable){
    ...
    if(keydown){
        ...
        if(printable)
            ui->pteMacroText->insertPlainText(line);
        else
            ui->pteMacroText->insertPlainText("<" + line + ">");
    }
}
...
QString RebindWidget::macroLinesToString()
{
    QString l;
    ...
    for(int i = 0; i < mlength; i++){
        ...
        if(i + 1 < mlength)
            l.append("=" + QString::number(macroLines.at(i+1).usTime) + ",");
    }

    return l;
}

void RebindWidget::on_pushButton_clicked()
{
    MacroStringEditDialog* dlg = new MacroStringEditDialog(macroLinesToString(), this);
